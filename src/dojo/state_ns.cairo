...
pub impl StateNsBeaconEmitter<
    TState, impl Emitter: HasEmitterComponent<TState>, +Drop<TState>,
> of BeaconEmitterTrait<NsContractState<TState>> {
    fn emit_model<M, +Model<M>>(ref self: NsContractState<TState>, model: @M) {
    ...
    fn emit_models<M, +Model<M>>(ref self: NsContractState<TState>, models: Span<M>) {
        let mut emitter: EmitterState = self.contract_state.get_component_mut();
    ...
    fn emit_entity<M, T, +Model<M>, +Serde<T>>(
        ref self: NsContractState<TState>, entity_id: felt252, entity: @T,
    ) {
        let mut emitter: EmitterState = self.contract_state.get_component_mut();
        emitter
            .emit_update_record(
    ...
    fn emit_entities<M, T, +Model<M>, +Serde<T>>(
        ref self: NsContractState<TState>, entities: Array<(felt252, @T)>,
    ) {
        let mut emitter: EmitterState = self.contract_state.get_component_mut();
        let selector = Model::<M>::selector(self.namespace_hash);
    ...
    fn emit_member<M, T, +Model<M>, +Serde<T>>(
        ref self: NsContractState<TState>, entity_id: felt252, member_selector: felt252, member: @T,
    ) {
        let mut emitter: EmitterState = self.contract_state.get_component_mut();
        emitter
            .emit_update_member(
    ...
    fn emit_members<M, T, +Model<M>, +Serde<T>>(
        ref self: NsContractState<TState>, member_selector: felt252, members: Array<(felt252, @T)>,
    ) {
        let mut emitter: EmitterState = self.contract_state.get_component_mut();
        let selector = Model::<M>::selector(self.namespace_hash);
    ...
    fn emit_model_members<M, T, +Model<M>, +Serde<T>>(
        ref self: NsContractState<TState>, entity_id: felt252, members: Array<(felt252, @T)>,
    ) {
        let mut emitter: EmitterState = self.contract_state.get_component_mut();
        let selector = Model::<M>::selector(self.namespace_hash);
    ...
    fn emit_delete_model<M, +Model<M>>(ref self: NsContractState<TState>, entity_id: felt252) {
        let mut emitter: EmitterState = self.contract_state.get_component_mut();
        emitter.emit_delete_record(Model::<M>::selector(self.namespace_hash), entity_id);
    }
}
